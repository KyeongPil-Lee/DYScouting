...
public:
  ...
  void Run()
  {
      ...
      if( DYTool::SelectGenEventBySampleType(sampleInfo_.type, ntuple) )
      {
        ...
        if( isDYEvent )
        {
          ...
          if (mvaValue > WP )
          {
            ...
            if( DYMuPair.isOS ) // -- only OS events
            {
              if( 10 < mass && mass < 30 )
                hists_below30->Fill(ntuple, DYMuPair, totWeight);

              else if ( 30 < mass && mass < 60 )
                hists_above30->Fill(ntuple, DYMuPair, totWeight);
            }
          } // -- end of MVA checking
        }
      }
    ...
    f_output->mkdir("below30");
    f_output->cd("below30");
    hists_below30->Write(f_output);

    f_output->mkdir("above30");
    f_output->cd("above30");
    hists_above30->Write(f_output);
