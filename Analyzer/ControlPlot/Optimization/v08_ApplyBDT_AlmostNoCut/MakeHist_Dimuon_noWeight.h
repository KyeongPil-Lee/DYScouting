...
// #include <BDT/dataset/weights/TMVAClassification_BDTG.class.C>
  ...
  void Run()
  {
    ...
    for(Int_t i=0; i<nEvent; i++)
    {
      ...
      if( DYTool::SelectGenEventBySampleType(sampleInfo_.type, ntuple) )
      {
        ...
        vector<DYTool::MuPair> vec_DYMuPair = DYTool::EventSelection_BDTInput_AlmostNoCut(ntuple, isDYEvent);
        if( isDYEvent )
        {
          vector<DYTool::MuPair> vec_selectedDYMuPair;
          for(auto& DYMuPair : vec_DYMuPair )
          {
            ...
            if( mvaValue > 0.009755 ) // -- 90% QCD pair rejection with 97.5 signal DY selection efficiency
              vec_selectedDYMuPair.push_back( DYMuPair );
          }

          // -- if there's more than one pair candidate passing MVA cut, select the one with the smallest vertex chi2
          if( vec_selectedDYMuPair.size() > 0 )
          {
            std::sort(vec_selectedDYMuPair.begin(), vec_selectedDYMuPair.end(), DYTool::CompareMuonPair_SmallerVtxChi2);

            DYTool::MuPair& selectedPair = vec_selectedDYMuPair[0];

            Double_t mass = selectedPair.mass;
            if( mass > 10 )
              hists->Fill(ntuple, selectedPair, totWeight);

            if( 60 < mass && mass < 120 )
              hists_ZPeak->Fill(ntuple, selectedPair, totWeight);

            if( 10 < mass && mass < 60 )
              hists_M10to60->Fill(ntuple, selectedPair, totWeight);
          }
        }
      }
    }
