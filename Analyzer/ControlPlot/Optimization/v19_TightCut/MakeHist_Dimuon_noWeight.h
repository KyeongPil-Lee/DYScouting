  ...
  void Run()
  {
    ...
    for(Int_t i=0; i<nEvent; i++)
    {
      ...
      if( DYTool::SelectGenEventBySampleType(sampleInfo_.type, ntuple) )
      {
        ...
        DYTool::MuPair DYMuPair = EventSelection_BDTInput_noOS_VeryTight(ntuple, isDYEvent, nPair);
...
private:
  DYTool::MuPair EventSelection_BDTInput_noOS_VeryTight(DYTool::DYTree *ntuple, Bool_t& doPass, Int_t &nPair)
  {
    doPass = kFALSE;
    ...
    for( auto& muPair : vec_muPair )
      if( muPair.IsDYCandidate_BDTInput_VeryTight(ntuple) ) vec_goodMuPair.push_back( muPair );
    ...
    else // -- multiple pair: the pair with the smallest chi2
    {
      doPass = kTRUE;
