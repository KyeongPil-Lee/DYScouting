...
public:
  ...
  void Run()
  {
    ...
    // -- additional info: dimuon mass distribution
    TH1D* h_diMuM_N           = new TH1D("h_diMuM",             "", 20, 81, 101);
    TH1D* h_diMuM_Nminus1_vtx = new TH1D("h_diMuM_Nminus1_vtx", "", 20, 81, 101);
    ...
    for(Int_t i=0; i<nEvent; i++)
    {
      ...
      if( DYTool::SelectGenEventBySampleType(sampleInfo_.type, ntuple) )
      {
        ...
        {
          h_nEvent_N->Fill( 0.5, totWeight ); // -- 0.5 = first bin
          h_diMuM_N->Fill( DYPair_N.mass, totWeight );
        }

        DYTool::MuPair DYPair_Nminus1_nPixelHit;
        DYTool::MuPair DYPair_Nminus1_nTrackerLayer;
        DYTool::MuPair DYPair_Nminus1_normChi2;
        DYTool::MuPair DYPair_Nminus1_relTrkIso;
        DYTool::MuPair DYPair_Nminus1_vtx;

        if( EventSelection_ZMassRange_N_minus_nPixelHit(ntuple, DYPair_Nminus1_nPixelHit) )         h_nEvent_Nminus1->Fill( 0.5, totWeight ); // -- 0.5 = first bin
        if( EventSelection_ZMassRange_N_minus_nTrackerLayer(ntuple, DYPair_Nminus1_nTrackerLayer) ) h_nEvent_Nminus1->Fill( 1.5, totWeight ); // -- 1.5 = second bin
        if( EventSelection_ZMassRange_N_minus_normChi2(ntuple, DYPair_Nminus1_normChi2) )           h_nEvent_Nminus1->Fill( 2.5, totWeight ); // -- 2.5 = 3rd bin
        if( EventSelection_ZMassRange_N_minus_relTrkIso(ntuple, DYPair_Nminus1_relTrkIso) )         h_nEvent_Nminus1->Fill( 3.5, totWeight ); // -- 3.5 = 4th bin

        if( EventSelection_ZMassRange_N_minus_CommonVertex(ntuple, DYPair_Nminus1_vtx) )
        {
          h_nEvent_Nminus1->Fill( 4.5, totWeight ); // -- 4.5 = 5th bin
          h_diMuM_Nminus1_vtx->Fill( DYPair_Nminus1_vtx.mass, totWeight );
        }
      }
    }
