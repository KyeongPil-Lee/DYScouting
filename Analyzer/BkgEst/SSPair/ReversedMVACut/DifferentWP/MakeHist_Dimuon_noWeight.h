...
public:
  ...
  void Run()
  {
    ...
    vector<TString> vec_tag = {"sigEff_10P", "sigEff_1P", "sigEff_0p1P"};
    vector<Double_t> vec_WP  = {0.004621,     -0.855961,   -0.974813};
    vector<HistContainer*> vec_hists;
    vector<HistContainer*> vec_hists_ZPeak;
    vector<HistContainer*> vec_hists_M10to60;
    vector<HistContainer*> vec_hists_M10to60_OS;
    vector<HistContainer*> vec_hists_M10to60_SS;
    Int_t nTag = (Int_t)vec_tag.size();
    for(Int_t i=0; i<nTag; i++)
    {
      vec_hists.push_back( new HistContainer() );
      vec_hists_ZPeak.push_back( new HistContainer() );
      vec_hists_M10to60.push_back( new HistContainer() );
      vec_hists_M10to60_OS.push_back( new HistContainer() );
      vec_hists_M10to60_SS.push_back( new HistContainer() );
    }
