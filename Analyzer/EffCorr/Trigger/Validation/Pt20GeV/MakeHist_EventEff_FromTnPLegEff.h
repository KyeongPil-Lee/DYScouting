...
public:
  ...
  void Run()
  {
    ...
    TH1D* h_diMuM_noEff   = new TH1D("h_diMuM_noEff",   "", 150, 50, 200);
    TH1D* h_diMuM_withEff = new TH1D("h_diMuM_withEff", "", 150, 50, 200);
    ...
    for(Int_t i=0; i<nEvent; i++)
    {
      ...
      if( DYTool::SelectGenEventBySampleType(sampleInfo_.type, ntuple) )
      {
        ...
        if( vec_genMuon[0].pt > 20 && fabs(vec_genMuon[0].eta) < 2.4 && 
            vec_genMuon[1].pt > 20 && fabs(vec_genMuon[1].eta) < 2.4)
        {
          vector<DYTool::OffMuon> vec_matchedOffMuon = DYTool::GenMatchedOfflineMuon(ntuple);
          if( vec_matchedOffMuon.size() != 2 ) continue;

          if( vec_matchedOffMuon[0].pt > 20 && fabs(vec_matchedOffMuon[0].eta) < 2.4 && vec_matchedOffMuon[0].isTRK &&
              vec_matchedOffMuon[1].pt > 20 && fabs(vec_matchedOffMuon[1].eta) < 2.4 && vec_matchedOffMuon[1].isTRK )
          {
