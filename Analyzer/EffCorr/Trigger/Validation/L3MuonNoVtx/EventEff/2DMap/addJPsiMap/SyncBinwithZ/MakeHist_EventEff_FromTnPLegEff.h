...
public:
  ...
  TH2D* h2D_eff_Z_lowPt_;
  TH2D* h2D_eff_Z_highPt_;

  TH2D* h2D_eff_JPsi_lowPt_;
  TH2D* h2D_eff_JPsi_highPt_;

  TnPEffMapTool(TString fileName_Z, TString fileName_JPsi)
  {
    fileName_Z_    = fileName_Z;
    fileName_JPsi_ = fileName_JPsi;
    Init();
  }
  ...
  Double_t GetEff(TString type, TString effType, Double_t pt, Double_t eta)
  {
    ...
    if( type == "lowPt" )
    {
      ...
      if( effType == "2D_pteta" ) eff = GetEff_GivenHist_2DMap( h2D_eff_Z_lowPt_, h2D_eff_JPsi_lowPt_, pt, eta );
    }
    else if( type == "highPt" )
    {
      ...
      if( effType == "2D_pteta" ) eff = GetEff_GivenHist_2DMap( h2D_eff_Z_highPt_, h2D_eff_JPsi_highPt_, pt, eta );
    }
    else
  ...
  // -- varX: should be pT
  Double_t GetEff_GivenHist_2DMap(TH2D* h2D_eff_Z, TH2D* h2D_eff_JPsi, Double_t varX, Double_t varY)
  {
    TH2D* h2D_eff = nullptr;

    if( varX < 10.0 ) h2D_eff = h2D_eff_JPsi;
    else              h2D_eff = h2D_eff_Z;
...
private:
  void Init()
  {
    h_eff_pt_lowPt_  = PlotTool::Get_Hist(fileName_Z_, "h_eff_pt_lowPt");
    h_eff_pt_highPt_ = PlotTool::Get_Hist(fileName_Z_, "h_eff_pt_highPt");

    h_eff_eta_lowPt_  = PlotTool::Get_Hist(fileName_Z_, "h_eff_eta_lowPt");
    h_eff_eta_highPt_ = PlotTool::Get_Hist(fileName_Z_, "h_eff_eta_highPt");

    h2D_eff_Z_lowPt_  = PlotTool::Get_Hist2D(fileName_Z_, "h2D_eff_lowPt");
    h2D_eff_Z_highPt_ = PlotTool::Get_Hist2D(fileName_Z_, "h2D_eff_highPt");

    h2D_eff_JPsi_lowPt_  = PlotTool::Get_Hist2D(fileName_JPsi_, "h2D_eff_lowPt");
    h2D_eff_JPsi_highPt_ = PlotTool::Get_Hist2D(fileName_JPsi_, "h2D_eff_highPt");
  }
};
...
public:
  ...
  void Run()
  {
    ...
    TString effFileName_TnP_Z    = basePath+"TnP/ROOTFile_L1HLTLegEfficiencyMap.root";
    TString effFileName_TnP_JPsi = basePath+"TnP/JPsi/SameBinWithZ/ROOTFile_L1HLTLegEfficiencyMap.root";
    TnPEffMapTool* effTool_TnP = new TnPEffMapTool(effFileName_TnP_Z, effFileName_TnP_JPsi);
