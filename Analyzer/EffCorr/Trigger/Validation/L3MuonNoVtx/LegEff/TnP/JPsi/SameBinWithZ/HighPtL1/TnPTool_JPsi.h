...
public:
  ...
  virtual Bool_t PassPairCondition()
  {
    ...
    if( 2.0 < mass_ && mass_ < 4.0 && deltaR > 0.3 )
      flag = kTRUE;
...
public:
  ...
  TnPHist( TString binType, Double_t minPtCut, vector<Double_t> vec_BinEdge )
  {
    ...
    // -- J/psi mass: 3.1 GeV ---> just set the mass range broad to have some margins
    // -- nMassBin should be also adjusted if the mass range is changed
    minM_ = 2.0;
    maxM_ = 4.0;
...
private:
  ...
  // -- mass range for tag&probe mass distribution
  Double_t minM_;
  Double_t maxM_;
  ...
  void GenerateHists()
  {
    ...
    Int_t nMassBin = 200; // -- bin size = 0.01 GeV 
...
private:
  ...
  void Init()
  {
    ...
    // minMass_ = 81;
    // maxMass_ = 101;
    minMass_ = 3.0; // -- narrower mass range than fitting (2.9 < m < 3.3 GeV)
    maxMass_ = 3.2;
    cout << "==========================================================" << endl;
