...
public:
  ...
  Double_t GetEff(Double_t pt, Double_t eta)
  {
    Int_t thePtBin = h_eff_pt_->GetXaxis()->FindBin(pt);

    // -- extrapolation: under and overflow
    if( thePtBin == 0 )       thePtBin = 1;
    if( thePtBin == nPtBin_+1 ) thePtBin = nPtBin_;

    return h_eff_pt_->GetBinContent(thePtBin);
  }
...
{
public:
  Bool_t debug_ = kFALSE; // -- limit # events running and print details
