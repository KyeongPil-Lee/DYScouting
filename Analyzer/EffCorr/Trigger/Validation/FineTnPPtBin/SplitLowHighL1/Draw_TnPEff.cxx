...
TString PtType(TString fileName)
{
  TString type = "";
  if( fileName.Contains("ROOTFile_MakeTnPHist_HighPtL1andHLT_DYMuMu_M50toInf.root") ) type = "highPtL1";
  if( fileName.Contains("ROOTFile_MakeTnPHist_LowPtL1andHLT_DYMuMu_M50toInf.root") )  type = "lowPtL1";
...
void Draw_TnPEff(TString fileName, TString var)
{
  ...
  TString ptType = PtType(fileName);
  TString canvasName = "c_TnPEff_"+var+"_"+ptType;
  ...
  if( var == "eta" )
  {
    if( ptType == "highPtL1" ) canvas->SetRangeY(0.76, 1.01);
    if( ptType == "lowPtL1" )  canvas->SetRangeY(0, 0.07);
  }
...
void Draw_TnP2DEff(TString fileName)
{
  ...
  TString ptType = PtType(fileName);
  TString canvasName = "c_TnP2DEff_"+ptType;
...
void Draw_TnPEff()
{
  TString fileName_highPt = "HighPt/ROOTFile_MakeTnPHist_HighPtL1andHLT_DYMuMu_M50toInf.root";
  TString fileName_lowPt  = "LowPt/ROOTFile_MakeTnPHist_LowPtL1andHLT_DYMuMu_M50toInf.root";

  Draw_TnPEff(fileName_highPt);
  Draw_TnPEff(fileName_lowPt);
