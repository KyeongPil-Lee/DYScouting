...
Bool_t PassTagCondition(DYTool::OffMuon mu, DYTool::DYTree* ntuple)
{
  ...
  if( DYTool::dRMatching_HLTObj(mu.vecP, ntuple, "hltL3crIsoL1sSingleMu22L1f0L2f10QL3f24QL3trkIsoFiltered0p07", 0.1) &&
      DYTool::dRMatching_HLTObj(mu.vecP, ntuple, "hltDoubleMu3L3FilteredNoVtx", 0.1) && // -- at least one muon should already pass DoubleMu3 filter; if not, DoubleMu3 filter does not pass even though the probe matched all conditions to pass the filter
      mu.pt > 26 &&
      fabs(mu.eta) < 2.4 &&
      mu.isTight && 
