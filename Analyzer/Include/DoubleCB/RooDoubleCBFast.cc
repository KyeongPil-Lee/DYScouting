...
//      double A1 = fast_pow(n1invalpha1,n1)*vdt::fast_exp(-alpha1*alpha1/2.);
//      double B1 = n1invalpha1-fabs(alpha1);
//      val = A1*fast_pow(B1-t,-n1);
   }else if(t>=alpha2){
...
//      double A2 = fast_pow(n2invalpha2,n2)*vdt::fast_exp(-alpha2*alpha2/2.);
//      double B2 = n2invalpha2-fabs(alpha2);
//      val = A2*fast_pow(B2+t,-n2);
   }//else{
 ...
 Double_t RooDoubleCBFast::analyticalIntegral(Int_t code, const char* rangeName) const 
 {
   ...
   if (isfullrange  && (n1-1.0)>1.e-5) {
	...
	double leftpow = fast_pow(n1invalpha1,-n1*invn1m1);
	double left0 = width*vdt::fast_exp(-0.5*alpha1*alpha1)*invn1m1;
	double left1, left2;
	
	if (xmax>(mean-alpha1*width)) left1 = n1invalpha1;
	else left1 = fast_pow( leftpow*(n1invalpha1 - alpha1 - thigh), 1.-n1);
	
	if (tmin<-1000.) left2 = 0.;
	else left2 = fast_pow( leftpow*(n1invalpha1 - alpha1 - tmin ), 1.-n1);
	
	left = left0*(left1-left2);
	
	//left = width*vdt::fast_exp(-0.5*alpha1*alpha1)*invn1m1*(n1invalpha1 -  fast_pow( fast_pow(n1invalpha1,-n1*invn1m1)*(n1invalpha1 - alpha1 - tmin), 1.-n1)) ;
	//left = width*vdt::fast_exp(-0.5*alpha1*alpha1)*invn1m1*(n1invalpha1 -  fast_pow( fast_pow(n1invalpha1,-n1*invn1m1)*(n1invalpha1 - alpha1 - tmin), 1.-n1)) ;
	//left = A1*vdt::fast_inv(-n1+1.0)*width*(fast_pow(B1-(left_low-mean)*invwidth,-n1+1.)-fast_pow(B1-(left_high-mean)*invwidth,-n1+1.));
      }
      else {
	double A1 = fast_pow(n1invalpha1,n1)*vdt::fast_exp(-0.5*alpha1*alpha1);
	double B1 = n1invalpha1-fabs(alpha1);	
   ...
   if (isfullrange && (n2-1.0)>1.e-5) {
	...
	double rightpow = fast_pow(n2invalpha2,-n2*invn2m2);
	double right0 = width*vdt::fast_exp(-0.5*alpha2*alpha2)*invn2m2;
	double right1, right2;
	
	if (xmin<(mean+alpha2*width)) right1 = n2invalpha2;
	else right1 = fast_pow( rightpow*(n2invalpha2 - alpha2 + tlow), 1.-n2);
	
	if (tmax>1000.) right2 = 0.;
	else right2 = fast_pow( rightpow*(n2invalpha2 - alpha2 + tmax), 1.-n2);
	
	right = right0*(right1-right2);	
	
	//right = A2*vdt::fast_inv(-n2+1.0)*width*(fast_pow(B2+(right_high-mean)*invwidth,-n2+1.)-fast_pow(B2+(right_low-mean)*invwidth,-n2+1.));
      }
      else {
	double A2 = fast_pow(n2invalpha2,n2)*vdt::fast_exp(-0.5*alpha2*alpha2);
	double B2 = n2invalpha2-fabs(alpha2);
