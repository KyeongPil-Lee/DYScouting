...
TH1D* MakeHist(TString histName, vector<Double_t> vec_binEdge, vector<Double_t> vec_binContent)
{
  ...
  TH1D* h = new TH1D(histName, "", nBin, arr_binEdge);
  for(Int_t i=0; i<nBin; i++)
  {
    Int_t i_bin = i+1;
    h->SetBinContent(i_bin, vec_binContent[i]);
...
void MakeMCPileupHist()
{
  vector<Double_t> vec_binEdge_2018 = {
  ...
  vector<Double_t> vec_binContent_2018 = {
  ...
  TH1D* h_PU_MC_2018 = MakeHist("h_PU_MC_2018", vec_binEdge_2018, vec_binContent_2018);


  TFile *f_output = TFile::Open("PileupHistogram_MC.root", "RECREATE");
  f_output->cd();
  h_PU_MC_2018->Write();
  f_output->Close();
}
