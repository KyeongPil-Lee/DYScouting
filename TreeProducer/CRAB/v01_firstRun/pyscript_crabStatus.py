...
for crabDir in CRABDirs:
    ...
    if "jobs failed " in stdout or "Could not find exit code details for" in stdout:
        ResubmtCMD += ['crab resubmit '+crabDirPath+' --proxy='+proxy]

    elif "COMPLETED" in stdout:
        CompletedList.append( crabDir )

    elif "TAPERECALL" in stdout:
        TapeRecallList.append( crabDir )
...
print "[TAPERECALL list]"
for one in TapeRecallList:
    print "'"+one+"',"
